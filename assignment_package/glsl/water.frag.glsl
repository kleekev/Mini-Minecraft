...
const float WARP_FREQUENCY = 1.0;
const float WARP_MAGNITUDE = 1.0;
const float NOISE_FREQUENCY = 4.0;

void main()
{
    // perform uv distortion in water
    // reference: https://gamedevelopment.tutsplus.com/tutorials/using-displacement-shaders-to-create-an-underwater-effect--cms-27191
    float X = fs_UV.x * 31.0 + u_Time * 0.01;
    float Y = fs_UV.y * 27.0 + u_Time * 0.01;
    vec4 baseColor = texture(u_Texture, vec2(fs_UV.x + sin(X-Y) * 0.005 * sin(Y), fs_UV.y + cos(X+Y) * 0.005 * cos(Y)));

    // add blue tint
    baseColor.b += (1 - baseColor.b) * 0.5;
    ...
    color = baseColor + vec4(0.1, 0.1, 0.1, 0) * noise;
}
